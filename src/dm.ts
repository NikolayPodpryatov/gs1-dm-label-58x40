...
export function normalizeRawInput(input: string, opts: NormalizeOpts = {}): string {
  ...
  // убрать мусорные переводы строк/табуляции
  s = s.replace(/[\r\n\t]+/g, '');

  // опционально: коррекция раскладки
  if (opts.ruToEn && typeof opts.ruToEnByLayout === 'function') {
    s = opts.ruToEnByLayout(s);
  }

  // убрать вариационные селекторы (emoji VS16 и т.п.)
  s = s.replace(/[\uFE0E\uFE0F]/g, '');

  // 0) Симвология-префиксы сканеров (в начале строки)
  s = s.replace(/^\](?:d2|D2|c1|C1)/, '');

  // 1) Лидирующий FNC1 в DM иногда «проступает» как U+00E8 ('è') или битый символ.
  // Удаляем ТОЛЬКО в начале (он там и должен быть как маркер GS1).
  s = s.replace(/^\u00E8/, '');         // 'è'
  // иногда попадается латинская 'ê' (редко), подстрахуемся:
  s = s.replace(/^\u00EA/, '');         // 'ê'

  // 2) Приведение заменителей к нашему GS
  s = s
    // явные плейсхолдеры
    .replace(/<GS>|\[GS\]|\^\]/gi, GS)
    // текстовые упоминания FNC1 как разделителя
    .replace(/\(FNC1\)|<FNC1>|FNC1|\\F/gi, GS)
    // символ ↔ (после удаления VS)
    .replace(/\u2194/g, GS)
    // реальный GS-контрольный символ
    .replace(/\u001D/g, GS)
    // литерал "\x1D"
    .replace(/\\x1d/gi, GS);

  // 3) Схлопываем подряд идущие GS
